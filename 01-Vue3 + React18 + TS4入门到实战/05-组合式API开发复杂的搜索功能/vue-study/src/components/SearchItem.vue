<template>
    ...
    <template v-if="searchType == 1">
        <div class="search-history">
            <div class="search-history-head">
                <span>历史记录</span>
                <i @click="handleClearHistory" class="iconfont iconlajitong"></i>
            </div>
            <div class="search-history-list">
                <div v-for="item in historyList" :key="item" @click="handleItemResult(item)">
                    {{ item }}
                </div>
            </div>
        </div>
    ...
    <template v-else-if="searchType == 2">
        <div class="search-result">
            <div v-for="item in resultList" :key="item.id" class="search-result-item">
    ...
    <template v-else-if="searchType == 3">
        <div class="search-suggest">
            ...
            >
                <i class="iconfont iconsearch">{{ item.name }}</i>
            </div>
        </div>
    </template>
</template>
...
<script setup>
    ...
    const { searchType, searchWord, handleToClose } = useSearch();
        ...
        const handleToSuggest = () => {
            if (searchWord.value) {
                ...
                axios.get(`api/search/suggest?keywords=${searchWord.value}`).then((res) => {
                    ...
                    for (const iterator of result.order) {
                        temp.push(...result[iterator]);
                    }

                    suggestList.value = temp;
                });
            } else {
                searchType.value = 1;
            }
    ...
    const { resultList, handleToResult, handleItemResult } = userResult();
